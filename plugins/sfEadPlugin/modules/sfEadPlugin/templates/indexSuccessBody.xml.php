...
<eadheader langencoding="iso639-2b" countryencoding="iso3166-1" dateencoding="iso8601" repositoryencoding="iso15511" scriptencoding="iso15924" relatedencoding="DC">
  ...
  <filedesc>
    ...
    <?php } ?>
    <?php if (('mods' != $defTemplate && 'dc' != $defTemplate) && $value = $resource->getRepository(['inherit' => true])) { ?>
      <publicationstmt>
...
<archdesc <?php echo $ead->renderLOD($resource, $eadLevels); ?> relatedencoding="<?php echo $template; ?>">
  <?php
    ...
    $topLevelDid = false;
  ?>
  <?php if (('mods' != $defTemplate && 'dc' != $defTemplate) && $resource->getPublicationStatus()) { ?>
  ...
  foreach ($radNotes as $name => $xmlType) {
      ...
      if (0 < count($notes = $resource->getNotesByType(['noteTypeId' => $noteTypeId]))) {
          foreach ($notes as $note) { ?>
            <?php if (('conservation' == $xmlType && ($authenticated || 1 == sfConfig::get('app_element_visibility_rad_conservation_notes'))) && ('conservation' == $xmlType && ($authenticated || 1 == sfConfig::get('app_element_visibility_rad_rights_notes')))) { ?>
              <odd type="<?php echo $xmlType; ?>" <?php if (0 < strlen($encoding = $ead->getMetadataParameter($xmlType))) { ?>encodinganalog="<?php echo $encoding; ?>"<?php } ?>><p><?php echo escape_dc(esc_specialchars($note->getContent(['cultureFallback' => true]))); ?></p></odd>
        <?php } ?>
      <?php }
      }
  ...
  <dsc type="combined">
    ...
    <?php foreach ($resource->getDescendantsForExport($options) as $descendant) { ?>
      ...
      <?php if (('mods' != $defTemplate && 'dc' != $defTemplate) && $descendant->getPublicationStatus()) { ?>
